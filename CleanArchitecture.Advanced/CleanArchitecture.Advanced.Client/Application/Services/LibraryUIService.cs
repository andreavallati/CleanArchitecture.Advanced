...
namespace CleanArchitecture.Advanced.Client.Application.Services
{
    public class LibraryUIService : ILibraryUIService
    {
        ...
        {
            return await ProcessItemsResponse<LibraryModel, LibraryDTO>(async () => await _libraryApiConnector.GetModels<LibraryDTO>("libraries"));
        }

        public async Task<ApiResponseItems<LibraryModel>> GetFilteredLibrariesAsync(SearchRequest filterRequest)
        {
            return await ProcessItemsResponse<LibraryModel, LibraryDTO>(async () => await _libraryApiConnector.PostModels<LibraryDTO, SearchRequest>("libraries/search", filterRequest));
        }

        public async Task<ApiResponseItems<LibraryModel>> GetWhereLibrariesAsync(GetWhereRequest getWhereRequest)
        {
            return await ProcessItemsResponse<LibraryModel, LibraryDTO>(async () => await _libraryApiConnector.PostModels<LibraryDTO, GetWhereRequest>("libraries/get-where", getWhereRequest));
        }

        public async Task<ApiResponseItem<LibraryModel>> GetLibraryByIdAsync(long libraryId)
        {
            return await ProcessItemResponse<LibraryModel, LibraryDTO>(async () => await _libraryApiConnector.GetModel<LibraryDTO>($"libraries/{libraryId}"));
        }

        public async Task<ApiResponseItem<LibraryModel>> FirstLibraryAsync()
        {
            return await ProcessItemResponse<LibraryModel, LibraryDTO>(async () => await _libraryApiConnector.GetModel<LibraryDTO>("libraries/first"));
        }

        public async Task<ApiResponseItem<LibraryModel>> FirstLibraryAsync(FirstEntityRequest firstEntityRequest)
        {
            return await ProcessItemResponse<LibraryModel, LibraryDTO>(async () => await _libraryApiConnector.PostModel<LibraryDTO, FirstEntityRequest>($"libraries/first", firstEntityRequest));
        }

        public async Task<ApiResponseItems<string>> SelectLibrariesNamesAsync()
        {
            return await ProcessItemsResponse<string, string>(async () => await _libraryApiConnector.GetModels<string>("libraries/select"));
        }

        public Task<ApiResponseItem<bool>> InsertLibraryAsync(LibraryModel library)
        {
            var model = _mapper.Map<LibraryDTO>(library);
            return ProcessItemResponse<bool, bool>(async () => await _libraryApiConnector.PostModel<bool, LibraryDTO>("libraries", model));
        }

        public async Task<ApiResponseItem<bool>> UpdateLibraryAsync(LibraryModel library)
        {
            var model = _mapper.Map<LibraryDTO>(library);
            return await ProcessItemResponse<bool, bool>(async () => await _libraryApiConnector.PutModel<bool, LibraryDTO>("libraries", model));
        }

        public async Task<ApiResponseItem<bool>> DeleteLibraryAsync(long libraryId)
        {
            return await ProcessItemResponse<bool, bool>(async () => await _libraryApiConnector.DeleteModel<bool>($"libraries/{libraryId}"));
        }

        private async Task<ApiResponseItem<TResult>> ProcessItemResponse<TResult, TSource>(Func<Task<ApiResponseItem<TSource>>> apiCall)
        {
            try
            {
        ...
        private async Task<ApiResponseItems<TResult>> ProcessItemsResponse<TResult, TSource>(Func<Task<ApiResponseItems<TSource>>> apiCall)
        {
            try
            {
                var response = await apiCall();
